		...
		case CHAT_MSG_OFFICER:
			{
				...
				if(m_muted && m_muted >= (uint32)UNIXTIME)
					{
						SystemMessage("Your voice is currently muted by a Game Master.");
						return;
					}

			if(GetPlayer()->m_modlanguage >=0)
			{
				data = sChatHandler.FillMessageData( CHAT_MSG_OFFICER, GetPlayer()->m_modlanguage,  msg.c_str(), _player->GetGUID(), _player->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM) ? 4 : 0 );
				_player->GetSession()->SendChatPacket(data, 1, 0, this); 
			}
			else
			{
				...
				data = sChatHandler.FillMessageData( CHAT_MSG_OFFICER, 0, msg.c_str(), _player->GetGUID(),_player->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM) ? 4 : 0 );
