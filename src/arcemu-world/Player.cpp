...
//			this->AddAuraVisual((*i).id,count_appearence-1,a->IsPositive());
	...
	if(IsDead())
	{
		BroadcastMessage("Revive function Available :");
		BroadcastMessage("Release Your character then use the teleporter to reive!");
		BroadcastMessage("*must be released or you'll end up bugged.");
		if(myCorpseInstanceId != 0)
		{
